...
contract BatchMintBadges is Script {
    ...
    function run() external {
        ...
        address[] memory allAddresses = abi.decode(vm.parseJson(json, ".addresses"), (address[]));
        ...
        for (uint256 i = 0; i < totalBatches; i++) {
            ...
            try manager.batchRegisterUsers(batchAddresses, initialGems, kycStatuses) {
    ...
    function runWithParams(uint256 _initialGems, bool _kycStatus, uint256 _startIndex, uint256 _endIndex) external {
        // Read and parse the JSON file
        string memory json = vm.readFile(ADDRESSES_FILE);
        address[] memory allAddresses = abi.decode(vm.parseJson(json, ".addresses"), (address[]));
        ...
        for (uint256 i = 0; i < totalBatches; i++) {
            ...
            console.log(string.concat("Processing batch ", vm.toString(i + 1), " of ", vm.toString(totalBatches)));
