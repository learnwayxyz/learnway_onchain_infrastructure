...
contract LearnWayBadgeComprehensiveTest is Test {
    ...
    function test_BadgeIDs_AllValidIDs() public {
        ...
        for (uint256 i = 2; i <= 24; i++) {
            if (i != 3) { // Skip Early Bird as it's auto-minted for KYC users
    ...
    function test_Badge1_Keyholder_SilverTierWithoutKYC() public {
        ...
        (bool hasBadge, uint256 tokenId, LearnWayBadge.BadgeTier tier,,) = 
    ...
    function test_AllBadges_HaveCorrectCategories() public view {
        ...
        for (uint256 i = 4; i <= 8; i++) {
            (, LearnWayBadge.BadgeCategory category,,,) = badgeContract.badges(i);
            assertEq(uint256(category), uint256(LearnWayBadge.BadgeCategory.QUIZ_COMPLETION), "Should be QUIZ_COMPLETION");
        }
        
        // Streaks & Consistency: 9-12
        for (uint256 i = 9; i <= 12; i++) {
            (, LearnWayBadge.BadgeCategory category,,,) = badgeContract.badges(i);
            assertEq(uint256(category), uint256(LearnWayBadge.BadgeCategory.STREAKS_CONSISTENCY), "Should be STREAKS_CONSISTENCY");
        }
        
        // Battles & Contests: 13-15
        for (uint256 i = 13; i <= 15; i++) {
            (, LearnWayBadge.BadgeCategory category,,,) = badgeContract.badges(i);
            assertEq(uint256(category), uint256(LearnWayBadge.BadgeCategory.BATTLES_CONTESTS), "Should be BATTLES_CONTESTS");
        }
        ...
        for (uint256 i = 20; i <= 22; i++) {
            (, LearnWayBadge.BadgeCategory category,,,) = badgeContract.badges(i);
            assertEq(uint256(category), uint256(LearnWayBadge.BadgeCategory.COMMUNITY_SHARING), "Should be COMMUNITY_SHARING");
        }
    ...
    function test_TokenURI_ContainsBadgeInfo() public {
        ...
        assertTrue(
            bytes(uri)[0] == bytes("d")[0] && 
            bytes(uri)[1] == bytes("a")[0] &&
            bytes(uri)[2] == bytes("t")[0] &&
